...
void calc_vorticity (vector<double> &wx_bin, vector<double> &wy_bin,
  ...
  for (int step = 0; step < nsteps; step++) {
	...
	if (ncells_per_bin[step][i*nbins+j] > 0) {
	  ...
	  int xfi = (i+1) % nbins;
	  int xbi = (i-1) % nbins;
	  wx_bin[step*nbins*nbins+i*nbins+j] = (vy_bin[step*nbins*nbins+xfi*nbins+j] - vy_bin[step*nbins*nbins+xbi*nbins+j])*rho;
	  
	  int yfi = (j+1) % nbins;
	  int ybi = (j-1) % nbins;
	  wy_bin[step*nbins*nbins+i*nbins+j] = (-vx_bin[step*nbins*nbins+i*nbins+yfi] + vx_bin[step*nbins*nbins+i*nbins+ybi])*rho;
	}
...
//   vector<vector<vector<double> > > vx_bin(nvels, vector<vector<double> >(nbins, vector<double>(nbins, 0.0)));
//   vector<vector<vector<double> > > vy_bin(nvels, vector<vector<double> >(nbins, vector<double>(nbins, 0.0)));
