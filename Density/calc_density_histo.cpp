...
vector<double> calc_densities (const double * const *x, const double * const *y,
  ...
  vector<double> densities(nbins*nbins, 0.);
  ...
  for (int step = 0; step < nsteps; step++) {
    for (int j = 0; j < ncells; j++) {
      ...
      int xbin = get_bin_number(x[step][j], lx, nbins);
      int ybin = get_bin_number(y[step][j], ly, nbins);
...
int main (int argc, char *argv[]) {
 ...
 // get the file name by parsing and load simulation data as well as positions
  ...
  cout << "Calculating density histogram for " << cells_or_beads << 
    " the following file: \n" << filename << endl;
  SimInfo sim(filename, cells_or_beads, get_image);  
  ...
  vector<double> densities = calc_densities(x, y, densities, 
					    sim.nsteps, sim.ncells);
