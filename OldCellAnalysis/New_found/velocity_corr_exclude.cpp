...
int main( int argc, char *argv[] ){
    ...
    for(int ti = 100; ti < T; ti++){                  // Time loop
    ...
    speed_per_cell_out << packing << "\t" << Fmc << "\t" << eps << "\t" << speed_per_cell_time_avg / (T-101) << endl;
    ...
    pres_per_cell_out << packing << "\t" << Fmc << "\t" << eps << "\t" << avg_pres_time/(T-100) << endl;
    ...
    mean_out << packing << "\t" << Fmc << "\t" << eps << "\t" << avg_mean_area/(T-100) << endl;
    mean_out.close();
    
    ofstream var_out;
    var_out.open( "avg_var_area_per_cell.txt", ios::out | ios::app );
    var_out << packing << "\t" << Fmc << "\t" << eps << "\t" << avg_var_area/(T-100) << endl;
    var_out.close();
    
    ofstream skew_out;
    skew_out.open( "avg_skew_area_per_cell.txt", ios::out | ios::app );
    skew_out << packing << "\t" << Fmc << "\t" << eps << "\t" << avg_skew_area/(T-100) << endl;
    skew_out.close();
    
    ofstream kurt_out;
    kurt_out.open( "avg_kurt_area_per_cell.txt", ios::out | ios::app );
    kurt_out << packing << "\t" << Fmc << "\t" << eps << "\t" << avg_kurt_area/(T-100) << endl;
    kurt_out.close();
    ...
    for(int i = 0; i < total_bins; i++){
        sp_vacf_out << i/(2*R_avg) << "\t" << cvv[i]/(denom_time_avg*(T-101)) << endl;
    }
    ...
    max_cluster_size_out << packing << "\t" << Fmc << "\t" << eps << "\t" << max_cluster_size_avg/(T-100) << endl;
    ...
    for(int di = 0; di < D; di++){              // Delay loop
        ...
        for(int m = 0; m < M; m++){             // Cell loop
            ...
            for(int ti = 100; ti < TD; ti++){        // Different time origins
