...
""" Plot area of the cells per frame per parameter"""
...
#            '$,\\kappa=$' + str(sim.kappa)
    ...
    line0 = ax0.scatter(x/sim.tau_D, data/sim.lx/sim.ly)
...
#        ", $t/\\tau_{A}$ = " + "{0:.2f}".format(time/sim.tau_A), fontsize=30)
    ...
    ax0.set_ylabel(r"$\langle \phi \rangle$", fontsize=40)
    ...
    if save_eps:
        savepath2 = base + savefolder + "_per_" + name + ".pdf"            
...
def main():
    ...
    parser.add_argument("-sf", "--savefolder", nargs="?", const="Area_per_frame", \
                        help="Specific folder for saving, as in Area_per_frame")   
