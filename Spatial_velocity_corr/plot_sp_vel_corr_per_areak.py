...
""" Plot spatial velocity correlation per area constraint potential parameter"""
...
#        ", $t/\\tau_{A}$ = " + "{0:.2f}".format(time/sim.tau_A), fontsize=30)
    ...
    ax0.set_ylabel("$C_{dd}$", fontsize=40)

    ### limits

    ax0.set_xlim((-1, sim.lx/2./sim.r_avg+1))
...
def main():
    ...
    parser.add_argument("-sf", "--savefolder", nargs="?", const="Sp_velocity_corr", \
                        help="Specific folder for saving, as in Sp_velocity_corr")   
