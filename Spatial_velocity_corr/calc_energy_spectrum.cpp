...
void calc_sp_vel_corr (double cvv[], 
		    ...
		    int delta, int ndata) {
  /* calculate spatial velocity correlation PER STEP */
  ...
  for (int step = 0; step < nsteps; step++) {
    ...
    for (int j1 = 0; j1 < ncells-1; j1++) {
      for (int j2 = j1+1; j2 < ncells; j2++) {
        ...
        cvv_per_step[ibin] += vx[step*ncells+j1]*vx[step*ncells+j2] + vy[step*ncells+j1]*vy[step*ncells+j2];
        cnn_per_step[ibin] += 1;
    ...
    for (int i = 0; i < ndata; i++) {
      if (cnn_per_step[i] != 0) {
	cvv[step*ndata+i] = cvv_per_step[i]/cnn_per_step[i];
      }
    }
...
void calc_energy_spectrum (double ek[], double kvec[],
  ...
  for (int step = 0; step < nsteps; step++) {
	...
	double dotp = kvec[k]*j;
	cost += cos(dotp)*cvv[step*ndata+j];    
	sint += sin(dotp)*cvv[step*ndata+j];    
...
/*    ek[j] = ek[j]/(nsteps*2*pi); */
    ek[j] = ek[j]*kvec[j]/(nsteps*2*pi); 
  }  
...
int main (int argc, char *argv[]) {
  ...
  */
  ...
  double cvv[nvels*longest_dist];
  for (int i = 0; i < nvels*longest_dist; i++) 
    cvv[i] = 0.; 
